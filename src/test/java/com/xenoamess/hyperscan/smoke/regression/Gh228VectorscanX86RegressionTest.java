...
public class Gh228VectorscanX86RegressionTest {
    ...
    @ParameterizedTest
    @Disabled("Known upstream issue: https://github.com/VectorCamp/vectorscan/issues/317")
    ...
    void shouldHaveAMatch(DualApi api) {
        ...
        List<String> regex = Arrays.asList(
                ...
                "^cop/devel/workflows-(prod|test)-.*/[0-9]+$",
        ...
        List<DualExpression> expressions = regex.stream()
                .map(expr -> api.createExpression(expr, flags))
        ...
        try (DualDatabase db = api.compileDatabase(expressions)) {
            try (DualScanner scanner = api.createScanner()) {
                ...
                List<DualMatch> matches = api.scan(scanner, db, "cop/devel/workflows-prod-build-cop-cop-ingestor/0");
